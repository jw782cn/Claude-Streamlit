...
Description: This module contains the implementation of a chatbot using OpenAI's GPT models.
Author: Chenran
"""
import time
...
ASK_TEMPLATE = """
You are a helpful assistant developed by OpenAI.
"""
...
class ChatGPT:
    ...
    def __init__(self, model=GPT_MODEL, temperature=0.1,
    ...
    def ask_llm(self, question, stream=False):
        ...
        chat_response = chat_completion_request(
            messages, model=self.model, temperature=self.temperature, tags=["chatgpt", self.model], stream=stream
        )
        # stream
        if stream:
            full_answer = ""
            for chunk in chat_response:
                ...
                if assistant_message.get("content"):
                    full_answer += assistant_message["content"]
